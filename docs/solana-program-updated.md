...
- **Minting Requires a Valid Attestation**

    Only attestations verified through Arcium confidential computation can authorize mints. The enclave authority public key is configured in BridgeConfig during initialization. Operators cannot forge deposits or mint arbitrary amounts because:
    1. Attestations must be signed by the trusted enclave
    2. Verification happens in confidential computing environment
    3. Callback is only triggered on successful verification
    4. PDA mint authority eliminates key management risks

- **No Note Reuse (Double-Spend Prevention)**

    Each Zcash note commitment is stored in a ClaimTracker PDA upon successful minting. The PDA seeds include the deposit intent address, ensuring one claim per intent. Once recorded, the note commitment cannot be used again. This prevents double-minting from a single deposit.

    ```rust
    // ClaimTracker PDA creation in verify_attestation_callback
    #[account(
        init,
        payer = payer,
...
- **Replay Protection on Burns**

    Burn intents are indexed by sequential `withdrawal_nonce` stored in BridgeConfig. Each burn gets a unique ID that increments atomically. Finalization events reference this burn_id, and MPC nodes track processed withdrawals off-chain. Replays cannot trigger additional withdrawals because:
    1. Tokens are burned immediately (supply reduced instantly)
    2. Burn events are indexed and cannot be duplicated
    3. MPC signature validation on finalization

- **Controlled Mint Authority**

    Only the Solana program holds mint authority for wZEC through a PDA. Users cannot increase supply on their own. The PDA-based authority means:
    1. No private key exists (impossible to compromise)
    2. Only program instructions can mint/burn
    3. All minting requires attestation verification

    SPL `burn` reduces both user balance and total mint supply. PDAs eliminate any need for private keys in deposit-intent or note commitment tracking.

- **Admin Role Limitations**

    The admin can:
    - Set unified addresses after generation (required for flow)
    - Call demo_mint for testing (should be disabled in production)

    The admin **cannot**:
    - Mint wZEC without valid attestation (except demo_mint)
    - Access user funds or reverse transactions
    - Modify completed deposit/withdrawal records
    - Bypass enclave attestation verification

- **Confidential Data Protection**

    Arcium confidential computing ensures:
    - Attestation verification logic cannot be observed
    - Withdrawal addresses remain private
    - Amounts are encrypted during transmission
    - Only authorized MPC nodes can decrypt burn intents
    - On-chain observers see only ciphertext and nonces

- **Multi-Party Computation (MPC) Integration**

    The bridge stores `mpc_quorum_pubkeys` in BridgeConfig for coordination with off-chain MPC nodes responsible for:
    - Generating Zcash Unified Addresses
    - Monitoring Zcash blockchain for deposits
    - Signing Zcash withdrawal transactions
    - Finalizing withdrawals on Solana

    The MPC quorum design distributes trust and prevents single-node compromise.

---

## **4.1.6 Program Constants**
...
// Account sizes
BridgeConfig::MAX_SIZE = 8 + 1 + 32 + 32 + 32 + 1 + 8 + 8 + (4 + 32*10) + (4 + 512);
DepositIntent::MAX_SIZE = 8 + 1 + 8 + 32 + 4 + 1 + 8 + 32 + 256 + 2 + 8;
ClaimTracker::SIZE = 8 + 1 + 32 + 8 + 8;

// Mint decimals
WZEC_DECIMALS = 8;  // 1 ZEC = 100,000,000 zatoshis
```
...
pub struct TokensMinted {
    ...
    pub user: Pubkey,
    pub amount: u64,
    pub encrypted_attestation: [u8; 32],  // Ciphertext for auditing
}

#[event]
pub struct BurnIntentCreated {
    pub burn_id: u64,
    pub encrypted_burn_data: [u8; 32],  // Encrypted: user, amount, address
    pub nonce: [u8; 16],
}
...
pub struct WithdrawalFinalized {
    pub burn_id: u64,
    pub encrypted_txid: [u8; 32],  // Encrypted Zcash TXID
    pub nonce: [u8; 16],
}
```
...
// Step 2: Wait for admin to set UA (off-chain API call)
const ua = await fetchUnifiedAddress(depositId);

// Admin calls set_unified_address
await program.methods
  .setUnifiedAddress(depositId, Array.from(ua))
  .accounts({
...
// Step 3: User sends ZEC to UA, receives attestation from enclave
const attestation = await getEnclaveAttestation(depositId);

// Step 4: Submit attestation for minting
await program.methods
  .mintWithAttestation(
    computationOffset,
    depositId,
    attestation.encrypted_data,
    attestation.pub_key,
    attestation.nonce
  )
  .accounts({
  ...
  .signers([userKeypair, payerKeypair])
  .rpc();

// Arcium automatically calls verify_attestation_callback
// wZEC is minted to user's token account
```

### **Complete Withdrawal Flow**

```typescript
// Step 1: Burn wZEC and create withdrawal intent
const burnTx = await program.methods
  .burnForWithdrawal(
    computationOffset,
    amount,
    Array.from(zcashAddress),
    encryptedData,
    pubKey,
    nonce
  )
...
// Step 2: MPC nodes detect BurnIntentCreated event, create Zcash TX

// Step 3: MPC finalizes after broadcasting Zcash transaction
await program.methods
  .finalizeWithdrawal(
    computationOffset,
    burnId,
  ...
  .accounts({
    mpcNode: mpcNodeKeypair.publicKey,
    payer: payerKeypair.publicKey,
    bridgeConfig: bridgeConfigPDA,
    // ... Arcium accounts
  })
  .signers([mpcNodeKeypair, payerKeypair])
  .rpc();

// WithdrawalFinalized event emitted with encrypted TXID
```
